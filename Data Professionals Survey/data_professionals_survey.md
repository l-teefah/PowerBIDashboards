...
This README documents the process of creating the Power BI dashboards for the Data Professionals Survey analysis.

## Overview

This project analyzes survey data from 630 data professionals across various roles, countries, and experience levels. The dashboard provides insights into:
- Demographics (gender, country of residence, age)
- Job roles and salaries
- Programming language preferences
- Work satisfaction metrics (work/life balance, salary happiness)
...
The following DAX measures were created to enable advanced analytics and visualization features:

### Core Metrics

```dax
# Total Respondents
Total Respondents = COUNT(Data[ID])

# Average Salary
Average Salary = AVERAGE(Data[Salary])

# Gender Distribution
Male Count = COUNTROWS(FILTER(Data, Data[Gender] = "Male"))
Female Count = COUNTROWS(FILTER(Data, Data[Gender] = "Female"))
```

### Quadrant Analysis Measures

```dax
# Horizontal Reference Line (Salary Threshold)
Salary Threshold Line = 
VAR SelectedThreshold = SELECTEDVALUE(SalaryThreshold[Value], 112)
RETURN SelectedThreshold

# Vertical Reference Line (Happiness Threshold)
Happiness Threshold Line = 
VAR SelectedThreshold = SELECTEDVALUE(HappinessThreshold[Value], 5)
RETURN SelectedThreshold

# Quadrant Assignment
Quadrant = 
VAR CurrentSalary = [Average Salary]
VAR CurrentHappiness = [Happiness with Salary]
VAR SalaryThreshold = [Salary Threshold Line]
VAR HappinessThreshold = [Happiness Threshold Line]
RETURN
    SWITCH(
        TRUE(),
        CurrentSalary >= SalaryThreshold && CurrentHappiness >= HappinessThreshold, "Q1: High Salary, High Happiness",
        CurrentSalary < SalaryThreshold && CurrentHappiness >= HappinessThreshold, "Q2: Low Salary, High Happiness",
        CurrentSalary < SalaryThreshold && CurrentHappiness < HappinessThreshold, "Q3: Low Salary, Low Happiness",
        CurrentSalary >= SalaryThreshold && CurrentHappiness < HappinessThreshold, "Q4: High Salary, Low Happiness"
    )
```
...
```dax
# Satisfaction Ratio
Satisfaction to Salary Ratio = 
DIVIDE([Happiness with Salary], [Average Salary]) * 100

# Salary Premium by Role
Salary Premium = 
VAR RoleAvg = [Average Salary]
VAR OverallAvg = CALCULATE([Average Salary], ALL(Data[Job Role]))
RETURN
    DIVIDE(RoleAvg - OverallAvg, OverallAvg) * 100
```
